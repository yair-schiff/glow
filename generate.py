...
def main(hps):
    ...
    print("sampling")
    n_batch = 10000
    y = np.asarray([_y % 10 for _y in (
            list(range(n_batch)))], dtype='int32')

    x_samples = []
    start_time = time.time()
    for i in range(1):
        x_samples.append(sample_n_batch(model, y, n_batch, [.7]*n_batch)[:, 2:30, 2:30, 0])
        print(x_samples[0].shape)
    print(time.time()-start_time)
    final_sample = np.concatenate(x_samples, axis = 0)
    np.save("glow-mnist.npy", final_sample)


def sample_n_batch(model, y, n_batch, eps):
        xs = []
